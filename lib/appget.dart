...
class _AppGetState extends State<AppGet> {
  ...
  setState(() {
  _products = json.decode(response.body); // atualiza estado do widget com a lista de produtos da API
  });
  } else {
  ...
  if (response.statusCode == 200) {
  setState(() {
  _products.removeWhere((product) => product['id'] == id); // remove produto da lista atualizando o estado do widget
  });
  ...
  children: <Widget>[
  Text('Categoria: ' + _products[index]['category']),
  Text('Descrição: ' + _products[index]['description']),
  Text('Preço: ' + _products[index]['price'].toString()),
  Row(
  mainAxisAlignment: MainAxisAlignment.end,
  children: <Widget>[
  IconButton(
  onPressed: () async {
  await _deleteProduct(_products[index]['id']);
  },
